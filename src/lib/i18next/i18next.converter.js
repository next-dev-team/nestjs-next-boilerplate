...
files.forEach(file => {
  ...
  // Read file and map json key combine with namespace
  const str = readFileSync(resolve(directoryPath, file));
  xdata = { ...xdata, ...JSON.parse(str) };
  const d = Object.keys(xdata).map(d => (defaultNamespace ? d : `${namespace}:${d}`));
  xkeys.push(...d);
});

// Text with initial information
let xtext = `/* tslint:disable */
/* eslint-disable */
// This file was automatically generated and should not be edited.
...
// Write text into typing file back
const xresultFile = resolve('.', 'src', 'common', 'types', 'translate.ts');
writeFileSync(xresultFile, xtext);
